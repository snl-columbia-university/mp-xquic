...
static quic_ctx_t *g_stm_ctx = NULL;
...
// GAME socket callback
static void stm_game_read_cb(int fd, short what, void *arg) {
    quic_ctx_t *ctx = (quic_ctx_t *)g_stm_ctx;
    unsigned char buf[2000];
    struct sockaddr_in src_addr;
    socklen_t src_len = sizeof(src_addr);

    // recv from GAME socket
    while (1) {
...
static int server_accept(xqc_engine_t *eng, xqc_connection_t *conn,
    ...
    // create GAME socket
    ctx->game_fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (ctx->game_fd >= 0) {
        fcntl(ctx->game_fd, F_SETFL, O_NONBLOCK);
        struct sockaddr_in stm_local_addr;
        memset(&stm_local_addr, 0, sizeof(stm_local_addr));
        stm_local_addr.sin_family = AF_INET;
        stm_local_addr.sin_port = htons(0); 
        stm_local_addr.sin_addr.s_addr = htonl(INADDR_ANY);
        
        if (bind(ctx->game_fd, (struct sockaddr *)&stm_local_addr, sizeof(stm_local_addr)) < 0) {
            perror("[server-stm] bind backend socket port failed");
        }

        // add GAME socket to libevent loop
        struct event *stm_ev = event_new(eb, ctx->game_fd, EV_READ | EV_PERSIST, stm_game_read_cb, ctx);
        event_add(stm_ev, NULL);
        printf("[server-stm] listening for external backend traffic on port %d...\n", stm_local_addr.sin_port);
    }
...
static int conn_close_notify(xqc_connection_t *conn, const xqc_cid_t *cid, void *user_data, void *proto_data) { 
    ...
    if (ctx) {
        if (ctx->game_fd >= 0) close(ctx->game_fd);
        free(ctx);
    }
    if (g_stm_ctx == ctx) {
        g_stm_ctx = NULL;
...
static void datagram_read_notify(xqc_connection_t *conn, void *user_data, const void *data, size_t data_len, uint64_t flags) {
    ...
    if (ctx->game_fd) {
        ...
        */
        ...
        if (!is_new_datagram(client_datagram_id)) {
            printf("[server-quic] duplicate datagram %ld (<=%ld) recv from server\n", client_datagram_id, ctx->max_dgram_id);
            return;
        }
