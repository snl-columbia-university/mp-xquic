...
def parse_line(line):
    ...
    if matches[1] in  ["server_listening", "connection_started", "connection_close", "connection_state_updated",
                        ...
                        "packet_lost", "http_parameters_set", "http_frame_created", "http_frame_parsed"]:
...
def parse_frames_processed(line):
    ...
    if 0 <= frame_type < len(frame_type_):
        type_str = frame_type_[frame_type]
        data["frames"][0]["frame_type"] = type_str.lower()
    else:
        type_str = f"UNKNOWN_{frame_type}"
        data["frames"][0]["frame_type"] = type_str.lower()
    ...
    if type_str == "PADDING":
        ...
        for i in range(1,len(segments)):
            ...
            if item[0] == "ack_range":
                ...
                pairs = s_clean.split(", ")  
                result = [list(map(int, pair.split(" - "))) for pair in pairs if pair]
                for i in range(len(result)):
                    if len(result[i]) == 2 and result[i][0] == result[i][1]:
                        result[i] = [result[i][0]]
