...
// citm config struct
static struct citm_config {
    const char *ctm_host;     
    int         ctm_ctrl_port;
    const char *servers_domain;  
    bool        multipath;  // allow switching targets after the first lock
} citm_cfg = {
    .ctm_host    = "cloud-traffic-manager.xrnet-columbia.com",
    .ctm_ctrl_port  = 5051,
    .servers_domain = "gameservers.xrnet-columbia.com",
    .multipath     = false,
};

// client id for registering with network api
static const char *client_id = "";


static quic_ctx_t *g_proxy_ctx = NULL;

static citm_ctx_t *citm_ctx = NULL;
...
static int resolve_servers(char paths[64][64]) {
    ...
    int rc = getaddrinfo(citm_cfg.servers_domain, NULL, &hints, &res);

    if (rc != 0) {
        LOG("telemetry: DNS resolution of '%s' failed: %s", citm_cfg.servers_domain, gai_strerror(rc));
        return 0;
    }

    int count = 0;

    // paths is owned by the caller (a single thread); no lock needed.
    ...
    freeaddrinfo(res);
    if (count == 0) {
        LOG("telemetry: '%s' resolved to no IPv4 addresses", citm_cfg.servers_domain);
    }
    else { 
        LOG("telemetry: resolved %d server(s) from '%s'", count, citm_cfg.servers_domain);
    }
    return count;
}

// Returns RTT in ms, -1.0 on no reply (timeout/unreachable), or -2.0 if the
// ping command could not be spawned.
...
static void *run_proxy_thread(void *arg) {

    pthread_mutex_lock(&citm_ctx->lock);

    while (!citm_ctx->ready) { 
        pthread_cond_wait(&citm_ctx->cond, &citm_ctx->lock); 
    }

    int bind_port = citm_ctx->game_port;
    
    pthread_mutex_unlock(&citm_ctx->lock);

    int fd = socket(AF_INET, SOCK_DGRAM, 0);

    if (fd < 0) { 
        LOG("proxy: socket() failed: %s", strerror(errno)); 
        return NULL; 
    }
    ...
    struct sockaddr_in addr = { .sin_family = AF_INET, .sin_port = htons(bind_port) };

    addr.sin_addr.s_addr = INADDR_ANY;

    if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        LOG("proxy: bind on port %d failed: %s", bind_port, strerror(errno));
        close(fd);
        return NULL;
    }

    LOG("UDP proxy listening on 0.0.0.0:%d", bind_port);
    
    struct sockaddr_in client_addr;
    socklen_t client_len = 0;
    bool has_client = false;
    unsigned char buf[6 + 4096];

    while (1) {
        struct sockaddr_in src;
        socklen_t src_len = sizeof(src);

        // Leave room at the front of buf for the 6-byte header for stm purposes
        ssize_t n = recvfrom(fd, buf + 6, sizeof(buf) - 6, 0, (struct sockaddr *)&src, &src_len);
        ...
        if (is_loopback(src_ip)) {
            // From the local game client -> encapsulate and send to the STM.
            client_addr = src;
            client_len = src_len;
            has_client = true;
            ...
            struct sockaddr_in stm = { .sin_family = AF_INET, .sin_port = htons(citm_ctx->stm_port) };

            inet_pton(AF_INET, citm_ctx->stm_ip, &stm.sin_addr);

            struct in_addr game;

            if (inet_pton(AF_INET, citm_ctx->game_ip, &game) != 1) {
                pthread_mutex_unlock(&citm_ctx->lock);
                continue;
            }

            int game_port = citm_ctx->game_port;

            pthread_mutex_unlock(&citm_ctx->lock);

            // Build header [game_ip:4][game_port:2] in the 6 reserved bytes.
            memcpy(buf, &game.s_addr, 4);
            buf[4] = (unsigned char)((game_port >> 8) & 0xff);
            buf[5] = (unsigned char)(game_port & 0xff);
            if (sendto(fd, buf, n + 6, 0, (struct sockaddr *)&stm, sizeof(stm)) < 0) {
                LOG("proxy: forward to STM failed: %s", strerror(errno));
            }

        } else if (has_client) {
            if (sendto(fd, buf + 6, n, 0, (struct sockaddr *)&client_addr, client_len) < 0)
                LOG("proxy: reply to client failed: %s", strerror(errno));
        }
    }
    return NULL;
}
...
static void *publish_measurements_thread(void *arg) {
    ...
    // The server set comes from DNS, is fixed for the life of the process, and
    // is touched only by this thread. Resolve it once (retrying until the first
    // lookup succeeds) and ping that same set forever -- no lock, no snapshot.
    int count;
    ...
    while (1) {
        ...
        int len = snprintf(json, sizeof(json),
                           "{\"client_id\":\"%s\",\"measurements\":[", client_id);
        bool first = true;
        int ok = 0;
        for (int i = 0; i < count; i++) {
            double rtt = ping_host(paths[i]);
            if (rtt < 0) {
                if (reachable[i]) {  // log only on the up->down transition, not every cycle
        ...
        if (ok > 0) {  // nothing measured -> skip the empty report
            ...
            if (ctrl_sock != -1) {
                unsigned char op = CMD_TELEMETRY;
                bool werr = (write(ctrl_sock, &op, 1) < 0)
                         || (write(ctrl_sock, json, len) < 0)
                         || (write(ctrl_sock, "\n", 1) < 0);
                int e = errno;
                pthread_mutex_unlock(&sock_lock);
                if (werr) LOG("telemetry: send failed: %s (control thread will reconnect)", strerror(e));
            } else {
                pthread_mutex_unlock(&sock_lock);  // not connected; control thread logs that
            }
        }
        sleep(1);
    }
