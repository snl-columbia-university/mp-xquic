...
skip_path:
    ...
    if (scheduled_primary_path) {
        xqc_list_head_t *proactive_pos;
        
        xqc_list_for_each(proactive_pos, &conn->conn_paths_list) {
            xqc_path_ctx_t *path_item = xqc_list_entry(proactive_pos, xqc_path_ctx_t, path_list);
            
            /* 1. Skip checking if it's the already selected primary path */
            if (path_item == scheduled_primary_path) {
                xqc_log(conn->log, XQC_LOG_DEBUG,
                        "|proactive_redundancy_skip|conn:%p|pkt_num:%ui|path_id:%ui|reason:is_primary_path|",
                        conn, packet_out->po_pkt.pkt_num, path_item->path_id);
                continue;
            }

            /* 2. Check if the alternate path is fully up and running */
            if (path_item->path_state != XQC_PATH_STATE_ACTIVE) {
                xqc_log(conn->log, XQC_LOG_INFO,
                        "|proactive_redundancy_skip|conn:%p|pkt_num:%ui|path_id:%ui|reason:path_not_active|state:%d|",
                        conn, packet_out->po_pkt.pkt_num, path_item->path_id, path_item->path_state);
                continue;
            }

            /* 3. Check if the alternate path has congestion window/pacing room right now */
            int path_cwnd_allow = xqc_scheduler_check_path_can_send(path_item, packet_out, check_cwnd);
            
            xqc_log(conn->log, XQC_LOG_INFO,
                    "|proactive_redundancy_eval|conn:%p|pkt_num:%ui|path_id:%ui|cwnd_allow:%d|check_cwnd_flag:%d|",
                    conn, packet_out->po_pkt.pkt_num, path_item->path_id, path_cwnd_allow, check_cwnd);

            if (path_cwnd_allow) {
                /* Flag this path inside the 32-bit packet instruction mask */
                packet_out->po_experimental_redundancy_mask |= ((uint32_t)1 << path_item->path_id);
                
                xqc_log(conn->log, XQC_LOG_INFO,
                        "|proactive_redundancy_masked|conn:%p|pkt_num:%ui|replicated_to_path:%ui|current_mask:%ui|",
                        conn, packet_out->po_pkt.pkt_num, path_item->path_id, packet_out->po_experimental_redundancy_mask);
            }
        }
    }
